...
export default function ContentRows({ types }) {
  // ---------------------------------- dispatch ----------------------------
  const dispatch = useDispatch();
  const { typeFilters } = useSelector((state) => state.FirstModalSlice);

  // 1: filter types (mainCategory - subCategory)
  types = types.filter((type) =>
    typeFilters.mainCategoryId
      ? type.mainCategoryId == typeFilters.mainCategoryId
      : true
  );

  types = types.filter((type) =>
    typeFilters.subCategoryId
      ? type.subCategoryId == typeFilters.subCategoryId
      : true
  );
  ...
  return (
    <div id="results--row">
      ...
      {/* content rows */}
      {types.map((type) => (
        <div className="row g-0 align-items-center results--item" key={type.id}>
          <div className="col-2">
            <label className="col-form-label form--label row--label">
              {type.serial}
            </label>
          ...
          <div className="col-2 text-center">
            <button
              ...
              type="button"
              onClick={() =>
                dispatch(toggleEditTypeModal({ status: true, id: type.id }))
              }>
              <svg
            ...
            <button
              ...
              onClick={() =>
                dispatch(
                  toggleConfirmModal({
                    status: true,
                    targetURL: `${process.env.domainURL}/api/inner-types/${type.id}/delete`,
                    targetName: 'Inner-Type',
                  })
                )
              }>
              <svg
